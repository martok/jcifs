 ...
 * properties must be specified for jcifs to function (See
 * <a href="../../overview-summary.html#scp">Setting JCIFS Properties</a>). Here
 ...
 * <td>This URL references a share called <code>tmp</code> on the server
 ...
 * <td>A relativly sophisticated example that references a file
 * <code>msmith1</code>'s desktop as user <code>Administrator</code>. Notice the
 * '@' is URL encoded with the '%40' hexcode escape.</td>
 * </tr>
 * 
 * <tr>
 * <td width="20%"><code>smb://angus/</code></td>
 * <td>This references only a server. The behavior of some methods is different
 * in this context(e.g. you cannot <code>delete</code> a server) however as you
 ...
 * <td>This syntactically is identical to the above example. However if
 ...
 * <td>Just as <code>smb://server/</code> lists shares and
 ...
 * <td>The server name may also be a DNS name as it is in this example. See
 * <a href="../../../resolver.html">Setting Name Resolution Properties</a> for
 * details.</td>
 * </tr>
 * 
 * <tr>
 * <td width="20%"><code>smb://192.168.1.15/ADMIN$/</code></td>
 * <td>The server name may also be an IP address. See
 * <a href="../../../resolver.html">Setting Name Resolution Properties</a> for
 * details.</td>
 * </tr>
 * 
 ...
 * <td>A prototypical example that uses all the fields.</td>
 * </tr>
 * 
 * <tr>
 * <td width="20%"><code>smb://myworkgroup/angus/ &lt;-- ILLEGAL </code></td>
 * <td>Despite the hierarchial relationship between workgroups, servers, and
 ...
 * <td>URLs that represent workgroups, servers, shares, or directories require a
 * trailing slash '/'.</td>
 * </tr>
 *
 * <tr>
 * <td width="20%"><code>smb://MYGROUP/?SERVER=192.168.10.15</code></td>
 * <td>SMB URLs support some query string parameters. In this example the
 ...
 * <table border="1" cellpadding="3" cellspacing="0" width="100%">
 * <tr bgcolor="#ccccff">
 * <td colspan="3"><b>Examples Of SMB URLs When Augmented With A Second
 * Constructor Parameter</b></td>
 * <tr>
 * <td width="20%"><b>First Parameter</b></td>
 ...
 * </code></td>
 * <td><code>
 *  smb://myworkgroup/angus/ &lt;-- ILLEGAL<br>(But if you first create an <tt>SmbFile</tt>
 * with 'smb://workgroup/' and use and use it as the first parameter to a
 * constructor that accepts it with a second <tt>String</tt> parameter jCIFS
 * will factor out the 'workgroup'.) </code></td>
 * </tr>
 * 
 * </table>
 *
...
public class SmbFile extends URLConnection implements SmbConstants {
	...
	SmbFile(SmbFile context, String name, int type, int attributes, long createTime, long lastModified, long size,
			long lastAccess) throws MalformedURLException, UnknownHostException {
		this(context.isWorkgroup0() ? new URL(null, "smb://" + name + "/", Handler.SMB_HANDLER)
		...
		if (dr != null) {
			...
			do {
				try {
					...
					/*
					 * This is a key point. This is where we set the "tree" of
					 * this file which is like changing the rug out from
					 * underneath our feet.
					 */
					/*
					 * Technically we should also try to authenticate here but
					 * that means doing the session setup and tree connect
					 * separately. For now a simple connect will at least tell
					 * us if the host is alive. That should be sufficient for
					 * 99% of the cases. We can revisit this again for 2.0.
					 */
					trans.connect();
	...
	public void connect() throws IOException {
		if (isConnected() && tree.session.transport.tconHostName == null) {
			/*
			 * Tree thinks it is connected but transport disconnected under it,
			 * reset tree to reflect the truth.
			 */
		...
		for (;;) {
			try {
				...
				throw sae; // Prevents account lockout on servers with multiple
							// IPs
			} catch (SmbException se) {
	...
	public boolean exists() throws SmbException {
		...
		try {
			if (url.getHost().length() == 0) {
				...
				accessTime = info.getLastAccessTime();
	...
	void doEnum(ArrayList list, boolean files, String wildcard, int searchAttributes, SmbFilenameFilter fnf,
	...
	void doShareEnum(ArrayList list, boolean files, String wildcard, int searchAttributes, SmbFilenameFilter fnf,
	...
	void doNetServerEnum(ArrayList list, boolean files, String wildcard, int searchAttributes, SmbFilenameFilter fnf,
			SmbFileFilter ff) throws SmbException, UnknownHostException, MalformedURLException {
	...
	void doFindFirstNext(ArrayList list, boolean files, String wildcard, int searchAttributes, SmbFilenameFilter fnf,
			SmbFileFilter ff) throws SmbException, UnknownHostException, MalformedURLException {
		SmbComTransaction req;
		...
		for (;;) {
			for (int i = 0; i < resp.numEntries; i++) {
				...
				if (name.length() > 0) {
					SmbFile f = new SmbFile(this, name, TYPE_FILESYSTEM, e.getAttributes(), e.createTime(),
							e.lastModified(), e.length(), e.lastAccess());
					if (ff != null && ff.accept(f) == false) {
						continue;
					}
					if (files) {
	...
	void copyTo0(SmbFile dest, byte[][] b, int bsize, WriterThread w, SmbComReadAndX req, SmbComReadAndXResponse resp)
			throws SmbException {
		int i;
		...
		if (isDirectory()) {
			...
			try {
				for (i = 0; i < files.length; i++) {
					ndest = new SmbFile(dest, files[i].getName(), files[i].type, files[i].attributes,
							files[i].createTime, files[i].lastModified, files[i].size, files[i].accessTime);
			...
			try {
				...
				dest.send(new Trans2SetFileInformation(dest.fid, attributes, createTime, lastModified, accessTime),
				...
				if (ignoreCopyToException == false)
					throw new SmbException(
							"Failed to copy file from [" + this.toString() + "] to [" + dest.toString() + "]", se);
	 ...
	 *
	 * @param time
	 *            the access time as milliseconds since Jan 1, 1970
	 */
	...
	public long getAccessTime() throws SmbException {
		if (getUncPath0().length() > 1) {
			exists();
			return accessTime;
		}
